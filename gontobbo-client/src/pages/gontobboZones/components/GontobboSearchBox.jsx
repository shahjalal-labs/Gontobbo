const GontobboSearchBox = () => {
  return (
    <div>
      

-- JSX Clipboard Auto-Paste into first matching empty component only
-- Paste this into your init.lua or any config Lua file
  ...
  for i = 1, max_files do
    ...
    if file:match("%.jsx$") or file:match("%.tsx$") then
      ...
      if component_name then
        ...
        -- Try single-line return <div></div>;
      ...
      if component_name then
        ...
        for lnum, line in ipairs(lines) do
          if line:match("return%s*<div>%s*</div>%s*;") then
            ...
            vim.cmd('normal! ""p')
            vim.cmd("write")
            return  -- Stop searching after first successful paste
          end
        end

        -- Try multiline return (
        --   <div>
        --   
        --   </div>
        -- );
        for lnum = 1, #lines - 4 do
          if lines[lnum]:match("^%s*return%s*%(%s*$")
            and lines[lnum + 1]:match("^%s*<div>%s*$")
            and lines[lnum + 3]:match("^%s*</div>%s*$")
            and lines[lnum + 4]:match("^%s*%)%s*;?%s*$") then
...
-- Optional autocmd: after deletes like dat/dst, auto paste after short delay
